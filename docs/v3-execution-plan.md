...
2026-09-22追記: 第3試作の許可確認に対するmaintainerの「再開して」に基づき、
固定block native adapterを追加する。元のBOM/model generator試作は保持する。
これは試作枠の個別追加であり、無制限の試作追加・P01再開・既定API採用の承認ではない。
契約・結果・残条件は[固定block試作](v3-fixed-block-pilot.md)に記録する。
